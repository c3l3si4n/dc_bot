...
async function sell_dirt() {
    ...
    bot.navigate.walk(path.path, async (stopReason) => {
        bot.dig(bot.blockAt(signDirt), async () => {
            bot.dig(bot.blockAt(signCoarsedDirt), () => {
                bot.chat("/pay ")
            });
        });
...
function check_limit(coord) {
    ...
    else if (blockOne.material == undefined && blockTwo.material == undefined && blockThree.material == undefined && blockFour.material == undefined && blockFive.material == undefined) {
        return false;
    }
    else {
        return true
